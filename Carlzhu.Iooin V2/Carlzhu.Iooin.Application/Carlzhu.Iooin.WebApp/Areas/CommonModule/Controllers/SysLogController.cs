...
namespace Carlzhu.Iooin.WebApp.Areas.CommonModule.Controllers
{
    ...
    /// 系统日志控制器
    /// </summary>
    public class SysLogController : PublicController<BaseSysLog>
    {
        private BaseSysLogBll Basesyslogbll = new BaseSysLogBll();
        /// <summary>
        /// 【系统日志】返回系统日志列表JSON
        /// </summary>
        ...
        /// <returns></returns>
        public ActionResult GridPageListJson(string ModuleId, string ParameterJson, Pagination jqgridparam)
        {
            try
            {
                ...
                //}
                List<BaseSysLog> ListData = Basesyslogbll.GetPageList(ModuleId, ParameterJson, ref jqgridparam);
                var JsonData = new
                {
        ...
        {
            List<BaseSysLogDetail> list = Basesyslogbll.GetSysLogDetailList(SysLogId);
        ...
        public ActionResult SubmitRemoveLog(string KeepTime)
        {
            ...
            {
                var Message = "清空失败。";
                int IsOk = BaseSysLogBll.Instance.RemoveLog(KeepTime);
                if (IsOk >= 0)
                {
                    IsOk = 1;
                    Message = "删除成功。";
                }
                BaseSysLogBll.Instance.WriteLog("", OperationType.Other, IsOk.ToString(), Message + Remark);
                return Content(new JsonMessage { Success = true, Code = IsOk.ToString(), Message = Message }.ToString());
            }
            catch (Exception ex)
            {
                BaseSysLogBll.Instance.WriteLog("", OperationType.Other, "-1", Remark + "," + "错误：" + ex.Message);
