...
//导出Excel
function btn_derive(sheetName) {
    SetDeriveExcel("#gridTable", sheetName);
    openDialog("/CommonModule/ExcelImport/DeriveDialog", "DeriveDialog", (sheetName+"导出"), 600, 300, function (iframe) {
...
function delConfirm(url, parm, msg) {
    confirmDialog("MINICUT", msg, function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
...
function delConfig(url, parm, count) {
    ...
    confirmDialog("MINICUT", "注：您确定要删除 " + count + " 笔记录？", function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
...
function GetDeriveExcel(obj, fileName) {
    ...
    confirmDialog("MINICUT", "注：您确定要导出 " + data.length + " 笔记录？", function (r) {
        if (r) {
            var postData = {
...
function BindDropItem(ControlId, Code, Memo) {
    ...
    getAjax('/CommonModule/DataDictionary/BinDingItemsJson', { Code: Code }, function (data) {
        ...
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i].Code).html(itemjson[i].Code + "-" + itemjson[i].FullName));
        });
        Loading(false);
    });
}
...
Memo:默认显示
*/
function BindDataBaseDropItem(ControlId, Code, Memo, url, filed, fileValue) {
    $(ControlId).html("");
    if (IsNullOrEmpty(Memo)) {
        $(ControlId).append("<option value=''>" + Memo + "</option>");
    }
    getAjax(url, { Code: Code }, function (data) {
        var itemjson = eval("(" + data + ")");
        $.each(itemjson, function (i) {
            $(ControlId).append($("<option></option>").val(itemjson[i][filed]).html(itemjson[i][filed] + "-" + itemjson[i][fileValue]));
        });
        Loading(false);
    });
}
...
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (tabs_container.find('li').length >= 8) {
            Loading(false);
            alertDialog("为保证系统效率,只允许同时运行8个功能窗口,请关闭一些窗口后重试！", 0)
            return false;
        }
