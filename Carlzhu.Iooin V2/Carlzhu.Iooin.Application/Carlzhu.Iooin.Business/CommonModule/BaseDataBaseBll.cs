...
namespace Carlzhu.Iooin.Business.CommonModule
{
    ...
    public class BaseDataBaseBll
    {
        ...
        /// <returns></returns>
        public DataTable GetDataTableList(string tableName, string ParameterJson, ref Pagination jqgridparam)
        {
            StringBuilder strSql = new StringBuilder();
            List<DbParameter> parameter = new List<DbParameter>();
        ...
        public void DeleteDbBackup(string KeyValue, string JobName)
        {
            ...
            try
            {
                int isOk = database.Delete<BaseBackupJob>(KeyValue, isOpenTrans);
                if (isOk > 0)
                {
                    //删除计划
                    List<DbParameter> parameter = new List<DbParameter>
                    {
                        DbFactory.CreateDbParameter("@JobName", JobName)
                    };
